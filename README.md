...
- OpenAI, Gemini, Groq, Anthropic, Ollama, OpenRouter, and NVIDIA NIM adapters.
- Per-request provider and model overrides.
- Shared response, usage, and tool-call normalization.

### REST API and sessions

- FastAPI app with chat, streaming chat, knowledge CRUD, semantic knowledge search, system info, health, metrics, and session endpoints.
...
```bash
curl http://127.0.0.1:8000/v1/knowledge
curl http://127.0.0.1:8000/v1/knowledge?limit=20&offset=0
```

Get a document's detail and chunk count:

```bash
curl http://127.0.0.1:8000/v1/knowledge/<document-id>
```

Semantic search preview (what RAG would retrieve):

```bash
curl -X POST http://127.0.0.1:8000/v1/knowledge/search \
  -H 'Content-Type: application/json' \
  -d '{"query":"How does Doitall store memory?","limit":5}'
```

Delete a document and all its chunks:

```bash
curl -X DELETE http://127.0.0.1:8000/v1/knowledge/<document-id>
```
...
curl http://127.0.0.1:8000/v1/system/models

# List registered skills/tools
curl http://127.0.0.1:8000/v1/system/skills

# List connector, MCP, and plugin extensions
curl http://127.0.0.1:8000/v1/system/extensions

# Estimate token count before sending a chat request
curl -X POST http://127.0.0.1:8000/v1/tokens/estimate \
  -H 'Content-Type: application/json' \
  -d '{"text":"How much context does this use?"}'
```
...
- `web_search` — DuckDuckGo web search returning top result snippets.
- `web_fetch` — fetch and extract text from a URL, with SSRF protection for private/internal addresses.
...
- Doitall is currently a **single-tenant service**: one API key can access every session, memory, and knowledge record. Do not expose one deployment to mutually untrusted users.
- The built-in rate limiter is in-process and best suited for the default single-process deployment. Use Redis or another shared store before adding workers or replicas.
- HTTP metrics use route templates to avoid high-cardinality labels, but counters are process-local and reset on restart. Use a production metrics/OpenTelemetry backend before adding workers or replicas.
...
- Multi-provider LLM abstraction (OpenAI, Gemini, Groq, Anthropic, Ollama, OpenRouter, NVIDIA NIM).
- Persisted chat sessions and messages.
- Full knowledge CRUD API (ingest, list, detail, delete, semantic search).
- System endpoints for models, skills, extensions, and token estimation.
- Optional API-key auth, payload limits, rate limiting, request IDs, and production startup guardrails.
- Client-disconnect detection on SSE streams.
...
- Web frontend (see `docs/frontend-plan.md`).
